...
export interface IOptionChatGPT extends Document {
    userId: string;
    stock: string;
    action: 'CALL' | 'PUT';
    confidence: number;
...
const optionChatGPTSchema = new Schema(
    {
...
optionChatGPTSchema.index({ createdAt: -1 }, { expireAfterSeconds: 30 * 24 * 60 * 60 });

// Compound index for querying user recent insights
optionChatGPTSchema.index({ userId: 1, stock: 1, createdAt: -1 });

export const OptionChatGPT = mongoose.model<IOptionChatGPT>('OptionChatGPT', optionChatGPTSchema);
