...
export interface IOptionClaude extends Document {
    userId: string;
    stock: string;
    action: 'CALL' | 'PUT';
    confidence: number;
...
const optionClaudeSchema = new Schema(
    {
...
optionClaudeSchema.index({ createdAt: -1 }, { expireAfterSeconds: 30 * 24 * 60 * 60 });
optionClaudeSchema.index({ userId: 1, stock: 1, createdAt: -1 });

export const OptionClaude = mongoose.model<IOptionClaude>('OptionClaude', optionClaudeSchema);
